...
fn setup<'a>(
    ...
    (client, env, admin, source, listener)
}
...
fn test_subscribe_registers_listener() {
    ...
    let (client, _env, _admin, source, listener) = setup(&env);
...
fn test_subscribe_emits_subscription_event() {
    ...
    let found_subscription_event = events
        .iter()
        .any(|(contract_address, _topics, _data)| *contract_address == client.address);
    assert!(
...
fn test_unsubscribe_emits_unsubscribe_event() {
    ...
    let events = env.events().all();
    let found_unsubscribe_event = events
        .iter()
        .any(|(contract_address, _topics, _data)| *contract_address == client.address);
    assert!(
...
fn test_notify_dispatches_to_single_subscriber() {
    ...
    let (client, _env, _admin, source, _listener) = setup(&env);
...
fn test_notify_payload_shape_matches_interface() {
    ...
    let notification = Notification {
        source: source.clone(),
        event_type: symbol_short!("yield_acrd"),
        data: data.clone(),
    };
...
fn test_notify_respects_event_type_filter() {
    ...
    let withdrawal_notification = make_notification(&env, &source, symbol_short!("withdraw"));
...
fn test_notify_emits_notification_emitted_event() {
    ...
    let events = env.events().all();
    let found_emitted_event = events
        .iter()
        .any(|(contract_address, _topics, _data)| *contract_address == client.address);
    assert!(
        found_emitted_event,
...
fn test_initialize_emits_initialized_event() {
    ...
    let events = env.events().all();
    let found_initialized_event = events
        .iter()
        .any(|(contract_address, _topics, _data)| *contract_address == client.address);
    assert!(
...
fn test_double_initialize_fails() {
    ...
    let (client, _env, admin, _source, _listener) = setup(&env);
...
fn test_initialized_event_shape_matches_mapper() {
    ...
    let initialized_event = events
        .iter()
        .find(|(contract_address, _topics, _data)| *contract_address == client.address);
...
fn test_subscriptions_are_isolated_by_source() {
    ...
    let (client, _env, _admin, source1, listener) = setup(&env);
...
fn test_notify_only_dispatches_to_source_subscribers() {
    ...
    let (client, _env, _admin, source1, _listener) = setup(&env);
    let source2 = Address::generate(&env);
