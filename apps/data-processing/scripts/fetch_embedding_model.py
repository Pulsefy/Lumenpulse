#!/usr/bin/env python3
"""
Fetch and vendor the pinned spaCy embedding model at image build time.

This script installs the exact embedding model version declared in
``src/config/embedding_config.py`` so that the artifact is baked into the
container image instead of being downloaded at runtime. Containers built from
this image therefore start with no outbound network access for model
resolution.

Usage:
    python scripts/fetch_embedding_model.py [--check-only]
...
def _log(msg: str) -> None:
    print(f"[fetch_embedding_model] {msg}", flush=True)
...
def vendored_model_matches(cfg: EmbeddingConfig) -> bool:
...
def _install_model(cfg: EmbeddingConfig) -> None:
    ...
    """
    _log(f"Installing pinned embedding model {cfg.shipped_version_tag} from wheel...")
    install_target = os.environ.get(
        "EMBEDDING_MODEL_INSTALL_CMD",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Vendor the pinned spaCy embedding model."
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
    ...
    if vendored_model_matches(cfg):
        _log(
            f"Vendored embedding model {cfg.shipped_version_tag} is present "
            "and up-to-date."
        )
        return 0
    ...
    if args.check_only:
        _log(
            f"ERROR: expected embedding model {cfg.shipped_version_tag} at "
