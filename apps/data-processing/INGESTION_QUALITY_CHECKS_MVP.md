...
Dataset-level freshness and completeness targets are documented in
[INGESTION_DATASET_SLAS.md](INGESTION_DATASET_SLAS.md). Quality-check reports
include those targets plus the current dataset SLA measurements generated by the
run.
...
It also includes `dataset_sla_targets`, `dataset_sla_measurements`, and
`dataset_sla_breaches`.
